<div class="p-6">
    <div class="flex items-center justify-between mb-5">
        <div>
            ...
            <p class="text-sm text-gray-500">Tambah, edit, dan hapus jadwal pertandingan.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow p-4 mb-4 flex flex-col md:flex-row gap-3 items-center justify-between">
        <div class="w-full md:w-1/2">
            <input wire:model.live="search" type="text" placeholder="Cari tim / stadion..."
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-600">
                <tr>
                    ...
                    <th class="text-left p-3 cursor-pointer" wire:click="sortBy('home_team')">Home</th>
                    <th class="text-left p-3 cursor-pointer" wire:click="sortBy('away_team')">Away</th>
                    <th class="text-left p-3">Stadion</th>
                    <th class="text-center p-3 cursor-pointer" wire:click="sortBy('status')">Status</th>
            ...
            <tbody>
                @forelse($matches as $m)
                    <tr class="border-t">
                        <td class="p-3">{{ $m->match_date?->format('d M Y H:i') ?? '-' }}</td>
                        <td class="p-3 font-semibold">{{ $m->home_team }}</td>
                        <td class="p-3 font-semibold">{{ $m->away_team }}</td>
                        <td class="p-3 text-gray-600">{{ $m->stadium ?? '-' }}</td>
                        <td class="p-3 text-center">
                            @if($m->status === 'scheduled')
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">SCHEDULED</span>
                            @elseif($m->status === 'finished')
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">FINISHED</span>
                            @else
                                <span class="px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">CANCELLED</span>
                            @endif
                        </td>
                        <td class="p-3 text-right space-x-2">
                            <a href="{{ route('admin.matches.edit', $m->id) }}"
