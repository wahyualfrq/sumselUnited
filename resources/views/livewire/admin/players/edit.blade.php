<div class="p-6 max-w-4xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6 space-y-5">
        ...
        <div>
            ...
            <input wire:model.defer="player.name" type="text"
            ...
            @error('player.name') <div class="text-xs text-red-600 mt-1">{{ $message }}</div> @enderror
        </div>

        {{-- POSISI --}}
        <div>
            <label class="text-sm font-semibold">Posisi</label>
            <select wire:model.defer="player.position"
                class="mt-1 w-full px-4 py-2 rounded-lg border border-gray-200
                       focus:ring-2 focus:ring-rose-200 focus:border-rose-500 outline-none">
            ...
            @error('player.position') <div class="text-xs text-red-600 mt-1">{{ $message }}</div> @enderror
        </div>

        {{-- KATEGORI --}}
        <div>
            <label class="text-sm font-semibold">Kategori</label>
            <input wire:model.defer="player.category" type="text"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input wire:model.defer="player.number" type="number"
                    class="mt-1 w-full px-4 py-2 rounded-lg border border-gray-200">
            </div>

            <div>
                <label class="text-sm font-semibold">Usia</label>
                <input wire:model.defer="player.age" type="number"
                    class="mt-1 w-full px-4 py-2 rounded-lg border border-gray-200">
            </div>
        </div>
        ...
        <div>
            ...
            <input wire:model.defer="player.country" type="text"
                class="mt-1 w-full px-4 py-2 rounded-lg border border-gray-200">
        </div>
...
<div>
    ...
    @if ($player && $player->photo_url)
        <div class="mt-2">
            <img
                src="{{ asset('storage/' . $player->photo_url) }}"
